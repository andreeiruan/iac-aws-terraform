...
aws codecommit put-file --repository-name $REPO_NAME --branch-name master --commit-message "update artifacts" --file-path appspec.yaml --file-content $FILE_YML --file-mode NORMAL --region $AWS_REGION

sleep 30

COMMIT_ID=$(aws codecommit get-branch --repository-name $REPO_NAME --branch-name master | jq '.branch' | jq '.commitId' | tr -d '"')

aws codecommit put-file --repository-name $REPO_NAME --branch-name master --parent-commit-id $COMMIT_ID --commit-message "update artifacts" --file-path taskdef.json --file-content $FILE_JSON --file-mode NORMAL --region $AWS_REGION
